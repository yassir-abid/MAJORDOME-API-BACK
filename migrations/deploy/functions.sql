...
CREATE FUNCTION update_project_status(int) RETURNS void AS $$
    ...
    CASE
        WHEN 
            EXISTS (SELECT status FROM intervention WHERE project_id = $1 AND status = 'Programmée') 
            THEN 'Ouvert'
        WHEN 
            EXISTS (SELECT status FROM intervention WHERE project_id = $1 AND (status = 'Annulée' OR status = 'Terminée')) 
            THEN 'Clôturé'
        ELSE 'Vide'
        END
    WHERE id = $1
